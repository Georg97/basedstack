<script lang="ts">
	...
	import { ai, init, load, send, clearChat } from './engine.svelte';
	...
	/**
	 * Reusable on-device chat widget over the shared engine singleton
	 * (`$lib/ai/engine.svelte`) — drop it anywhere. `fill` grows the chat to its
	 * container (full-page use); otherwise the message area has a fixed height
	 * (embedding inside a section/card).
	 */
...
<div class={fill ? 'flex min-h-0 flex-1 flex-col' : ''}>
	...
	{:else}
		...
		<Card.Card class="bg-card/50 mb-4 border-white/[0.06]">
			<Card.CardContent class="p-4">
				<div class="flex flex-col gap-3 sm:flex-row sm:items-end">
					<div class="flex-1">
						<label for="basedstack-ai-model" class="text-muted-foreground mb-1.5 block text-xs font-medium tracking-wider uppercase">
							Model
						</label>
						<select
							id="basedstack-ai-model"
		...
		{#if ai.activeModel && (ai.phase === 'ready' || ai.phase === 'generating')}
			<Card.Card class="bg-card/50 flex flex-col overflow-hidden border-white/[0.06] {fill ? 'min-h-0 flex-1' : ''}">
				<div bind:this={scroller} class="space-y-3 overflow-y-auto p-4 {fill ? 'flex-1' : 'h-80'}">
